...
- We know that where the Delta variant is identified, it really rapidly takes off and spreads between people.

- Due to the Delta variant infections, there was a surge in cases across Victoria.

- Travel restriction were imposed and contact tracing became significant for detecting the spread of the virus. 
...
#map include for new cases 
 vic_lga_covid %>% 
   filter(!is.na(month)) %>% 
  ggplot() +
  geom_sf(mapping = aes(geometry= geometry, fill=lnew_cases)) +
  geom_sf_text(aes(geometry= geometry,label=NAME1, colour="white"), 
               check_overlap=TRUE)+
  #scale_fill_brewer(labels=colors)+
  theme_minimal() +
